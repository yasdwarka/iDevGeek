...
function submitForm(formDataObject, onComplete) {
  ...
  // place this line in the shared code experiment setup
...
function checkForLastSubmission(obj) {
  var url = isNetller ? 'https://news.neteller.com/pub/rf' : 'https://news.skrill.com/pub/rf';
  url += "?_ri_=" + obj._ri_;
  url += "&CUSTOMER_ID_=" + obj.CUSTOMER_ID_;
  url += "&CUSTOMER_ID=" + obj.CUSTOMER_ID;
  // var url = 'https://news.skrill.com/pub/rf?_ri_=X0Gzc2X%3DAQpglLjHJlTQGzccff3seLgg37YN3MBzgPIrzdhXkHTLC37acGYVwjpnpgHlpgneHmgJoXX0Gzc2X%3DAQpglLjHJlTQGq2LzcrWavJXhuOj1mzeLzbjizdhXkHTLC37acGY&CUSTOMER_ID_=191996645&CUSTOMER_ID=191996645'
  fetch(url, {
    method: 'GET',
    mode: 'no-cors',
    cache: 'no-cache',
    ...
    var _document = parser.parseFromString(html, "text/html");
    debugger
    var dateElement = _document.querySelector('input[name="LAST_SUBMITTED_DT"]');
    if (dateElement) {
      var date = dateElement.getAttribute('value')
      console.log(date)
    }
  }).catch(function(err) {
    // onComplete();
...
utils.waitForElement('#grid-main > pw-dashboard > div.pw-dashboard-spacing > div:nth-child(1)')
  .then(function(elem) {
    ...
    checkForLastSubmission(formDataObject)
